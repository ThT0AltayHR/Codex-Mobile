...
class CodexAuthManager(private val context: Context) {
    ...
    fun buildAuthorizeUrl(): String {
        ...
        val builder = Uri.parse(CodexAuthConfig.ISSUER + CodexAuthConfig.AUTHORIZE_PATH).buildUpon()
            ...
            // CRITICAL FIX: a prior version sent "codex_cli" here. The
            // real upstream Codex CLI sends "codex_cli_rs"
            // (DEFAULT_ORIGINATOR in codex-rs/login/src/auth/default_client.rs).
            // "codex_cli" is not in that file's recognized originator
            // allow-list at all. This mismatch is the most likely
            // explanation for "login appears to succeed but chat gets no
            // response, as if signed into ChatGPT instead of Codex" —
            // the backend's account/scope binding for the issued token
            // can depend on which originator requested it.
